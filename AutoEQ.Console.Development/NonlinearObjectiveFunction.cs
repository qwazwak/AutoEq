...
internal class NonlinearObjectiveFunction : IObjectiveModel
{
    ...
    public int DegreesFreedom => NumberOfObservations - NumberOfParameters;

    /// <summary>
    /// Get the degree of freedom
    /// </summary>
    public int DegreeOfFreedom => IsFixed != null ? DegreesFreedom + IsFixed.Count(p => p) : DegreesFreedom;
    ...
    public IObjectiveFunction ToObjectiveFunction()
    {
        ...
        ObjectiveFunction.Hessian(Function);
        var objective = new GradientHessianObjectiveFunction(Function);
        return objective;
    }*/
    public IObjectiveFunction ToObjectiveFunction()
    {
        (double, Matrix<double>) Function(Vector<double> point)
        {
            EvaluateAt(point);
            return (Value, Hessian);
        }
        return ObjectiveFunction.Hessian(Function);
    }
    ...
    Matrix<double> NumericalJacobian(Vector<double> parameters, Vector<double> currentValues, int accuracyOrder = 2)
    {
        ...
        for (int j = 0; j < NumberOfParameters; j++)
        {
            ...
            if (accuracyOrder >= 6)
            {
                ...
                Vector<double> prime = (-f1 + (9 * f2) - (45 * f3) + (45 * f4) - (9 * f5) + f6) / (60 * h[j]);
