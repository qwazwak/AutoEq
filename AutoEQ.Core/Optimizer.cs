...
{
    /// <summary>
    /// Minimize a function using Sequential Least Squares Programming Python interface function for the SLSQP Optimization subroutine originally implemented by Dieter Kraft.
    /// </summary>
    /// 
    /*
...
*/
    ...
    /// <param name=""></param>
    /// <param name=""></param>
    /// <param name="epsilon">float, optional, The step size for finite-difference derivative estimates.</param>
    /// <param name="callback">callable, optional Called after each iteration, as ``callback(x)``, where ``x`` is the current parameter vector.</param>
    /// <returns>
    ...
    ///  </returns>
    public static dynamic fmin_slsqp(func, x0, eqcons=(), f_eqcons=None, ieqcons=(), f_ieqcons=None,
               bounds=(), fprime=None, fprime_eqcons=None,
               fprime_ieqcons=None, args=(), iter=100, acc=1.0E-6,
               iprint=1, disp=None, bool full_output=true, epsilon=_epsilon,
               callback= None)
    {
    ...
    if f_ieqcons:
        cons += ({'type': 'ineq', 'fun': f_ieqcons, 'jac': fprime_ieqcons,
                  'args': args}, )
#endif
    //var res = _minimize_slsqp(func, x0, args, jac=fprime, bounds=bounds, constraints=cons, **opts);
    var res = _minimize_slsqp(func, x0, args, jac=fprime, bounds=bounds, **opts);
    ...
    /// <param name=""></param>
    /// <param name=""></param>
    /// <param name="unknown_options"></param>
    /// <returns></returns>
private static dynamic  _minimize_slsqp<T>(Func<T, object[], double>func, x0, args=(), jac=None, bounds=None,
    ...
    */
    ...
    for ic, con in enumerate(constraints):
        ...
        cons[ctype] += ({'fun': con['fun'],
                         ...
                         'args': con.get('args', ())}, )
    */
    ImmutableDictionary<int, string> exit_modes = ImmutableDictionary<int, string>.Empty
            .Add(-1, "Gradient evaluation required (g & a)")
            .Add(0, "Optimization terminated successfully")
            .Add(1, "Function evaluation required (f & c)")
            .Add(2, "More equality constraints than independent variables")
            .Add(3, "More than 3*n iterations in LSQ subproblem")
            .Add(4, "Inequality constraints incompatible")
            .Add(5, "Singular matrix E in LSQ subproblem")
            .Add(6, "Singular matrix C in LSQ subproblem")
            .Add(7, "Rank-deficient equality constraint subproblem HFTI")
            .Add(8, "Positive directional derivative for linesearch")
            .Add(9, "Iteration limit reached");
    ...
    // Initialize internal SLSQP state variables
        ...
        double[] f0 = new double[0];
        double[] h1 = new double[0];
        double[] h2 = new double[0];
        double[] h3 = new double[0];
        double[] h4 = new double[0];
        double[] t = new double[0];
        double[] t0 = new double[0];
        double[] tol = new double[0];
        int[] iexact = new int[0];
        int[] incons = new int[0];
        int[] ireset = new int[0];
        int[] itermx = new int[0];
        int[] line = new int[0];
        int[] n1 = new int[0];
        int[] n2 = new int[0];
        int[] n3 = new int[0];
...
/// </remarks>
public class OptimizeResult : Dictionary<string, dynamic>
{
    /*Attributes
    ----------
    x : ndarray
